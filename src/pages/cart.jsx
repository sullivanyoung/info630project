...
export const Cart = () => {
	...
	return (
		<>
					...
					{cart.map((item) => {
						...
						return (
							<tr className="text-center">
								...
								<td className="border border-green-800 px-4">${item.price}</td>
								<td className="border border-green-800 px-4">${(item.price * item.qty).toFixed(2)}</td>
								<th className="border border-green-800 px-4 hover:text-red-600">
									<button className="font-bold" onClick={() => buttonClick(item.id)}>
			...
			<h1 className="ml-5 py-5 font-bold">
			...
			<a
				key="checkout"
				href="/checkout"
				className="bg-green-400 hover:bg-green-700 text-black text-lg ml-5 px-3 py-2 rounded-md font-medium mr-2"
			>
				Checkout
			</a>
		</>
