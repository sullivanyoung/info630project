			...
			{cart.map((item, i) => {
				...
				return (
					<>
						...
						<h1>{item.price}</h1>
						<button
							className="bg-blue-500 hover:bg-blue-800 text-white font-bold py-2 px-10 rounded text-center focus:outline-none"
							onClick={
								(() => setCount(count - 1), () => setCart(cart.splice(i, 2)))
							}
						></button>
					</>
				);
			})}
			<h1>Cart Total: ${Math.ceil(total * 100) / 100}</h1>
		</>
	);
};
